...
- **Feature Detail Pages**: 6 dedicated educational pages at `/features/` (ai-agents, call-handling, compliance, analytics, multi-language, pay-per-talk-time). Each page includes hero section, visual data flow diagram, key benefits, real-world use cases, FAQ section, CTA, and prev/next feature navigation. Clickable from homepage feature grid cards. Designed for non-technical audience with plain language explanations. Navbar and Footer included on all pages.
- **Partner Pages**: Public partnership pages at `/partners` (hub), `/partners/whitelabel` (reseller/whitelabel programme), `/partners/affiliate` (affiliate referral programme). Hub page shows 3 partnership models with market opportunity, what GoRigo provides, general terms, and FAQ. Whitelabel page covers branding, what's included, requirements, eligibility, detailed T&Cs, and FAQ. Affiliate page covers commission flow, earnings, attribution, detailed T&Cs, and FAQ. All pages have Navbar/Footer, CTAs to /contact, and cross-links. Partners link added to Navbar and Footer.
- **Public Website**: Full marketing website with OpenRouter-inspired design. Homepage (`/`) with dark hero, capability highlights, 6 clickable feature cards linking to detail pages, 3-step how-it-works, industry use cases, pricing preview, and CTA sections. Shared Navbar and Footer components across all public pages. Additional pages: Pricing (`/pricing`) with 3 deployment packages and feature comparison table, About (`/about`) with company mission and UK registration details, Contact (`/contact`) with enquiry form and 24/7 AI demo line, Documentation hub (`/docs`) with API reference and integration guides. Four policy pages under `/policies/` (privacy, terms, cookies, acceptable-use) all referencing International Business Exchange Limited, Company No. 15985956. Visual sitemap (`/sitemap`) and XML sitemap (`/sitemap.xml`). SEO: unique metadata, Open Graph tags, robots.txt. Logo at `/logo.png`, favicon at `/favicon.png`. Email: hello@gorigo.ai.
