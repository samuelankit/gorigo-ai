...
async function seedWorkspace(
  ...
  if (existing.length > 0) {
    return { workspace: existing[0], skipped: true };
  }
  ...
  return { workspace, skipped: false, accounts: coa.length, taxCodes: DEFAULT_TAX_CODES.length };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const personal = await seedWorkspace(auth.orgId, "Personal", "personal", PERSONAL_COA);
    const company = await seedWorkspace(auth.orgId, "IBX", "company", COMPANY_COA, {
      companyName: "International Business Exchange Limited",
    });

    return NextResponse.json({
      success: true,
      personal: personal.skipped ? "Already exists" : `Created with ${personal.accounts} accounts and ${personal.taxCodes} tax codes`,
      company: company.skipped ? "Already exists" : `Created with ${company.accounts} accounts and ${company.taxCodes} tax codes`,
    });
  } catch (error) {
    return handleRouteError(error, "Finance Seed");
  }
}
