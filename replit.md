...
- **Billing & Usage**: Talk-time pricing, real-time wallet deductions, mid-call balance enforcement, spending caps, Stripe for top-ups, automated refunds, and call-end billing reconciliation (catches missed charges from server restarts). Active call billing state is persisted to `active_call_billing_state` DB table for crash resilience — billing resumes automatically after server restart via `restoreCallBilling()`. Files: `lib/mid-call-billing.ts`.
...
- **Campaign Execution Engine**: Automated outbound dialing engine (`lib/campaign-executor.ts`) polls running campaigns every 30s. Enforces pacing (`maxConcurrent`), budget caps (80% threshold auto-pause), calling hours, DNC/TPS compliance, and wallet balance checks. Atomic contact claiming prevents duplicate dials across instances. Wired into `lib/lazy-init.ts`.
- **Job Processor**: Background job queue (`lib/jobs.ts`) with `startJobProcessor()` processing pending jobs on interval (30s prod, 60s dev). Atomic job claiming via `status='pending'` guard prevents duplicate execution. Supports `CALL_BILLING`, `CALL_SUMMARY`, `DOCUMENT_CHUNK`, `DOCUMENT_EMBED`, `AUDIO_TRANSCRIBE`, and `CAMPAIGN_DIAL` job types.
- **TPS/CTPS Compliance**: UK Telephone Preference Service checking (`lib/tps-checker.ts`) with 30-day result caching in `tps_check_results` table. Integrated into `lib/compliance-engine.ts` for GB numbers when `dncCheckMethod === "api"`. Fails-open with warning when `TPS_API_KEY` is not configured.
