...
- **Rate Limiting**: All ~140 API endpoints protected with tiered rate limiters in middleware: auth mutations (15/min), auth reads (60/min), AI (20/min), Rigo (12/min middleware + 10/min route-level), public (15/min), knowledge (10/min), admin (30/min), exports (5/min), billing/wallet (10/min), settings (20/min), general (100/min). Twilio webhooks and Stripe webhook exempt. Body size limits per category (auth: 10KB, AI/test-chat: 100KB, public/rigo: 50KB, wallet/billing: 10KB, knowledge: 5MB, stripe-webhook: 1MB, default: 1MB).
...
- **Input Validation**: Zod schemas enforce strict validation on 50+ route handlers including auth, agents, knowledge, wallet, billing, campaigns, compliance, AI chat, Rigo, api-keys, onboarding, and settings. Sanitization utility at `lib/sanitize.ts`. Standardised error handler at `lib/api-error.ts` strips stack traces in production.
- **Sensitive Data Scrubbing**: Auth endpoints (me, login, register) strip passwordHash, emailVerificationToken, failedLoginAttempts, and lockedUntil from responses.
