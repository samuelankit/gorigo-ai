...
The platform includes a Unit Economics System for real-time cost tracking and margin analysis, with a pricing simulator. An optional Rigo Voice Assistant provides AI voice control over the SaaS dashboard. Security hardening includes audit logging, robust session management (2h idle timeout, 30d absolute expiry, 15-min token rotation, max 5 sessions per user, revoke individual/all sessions), database-backed rate limiting (PostgreSQL atomic upsert, fail-closed for sensitive buckets like auth/billing/admin), CSRF protection (origin/referer checking + sameSite:strict cookies), email verification enforcement on mutation routes, account lockout after 5 failed attempts, and comprehensive input validation using Zod schemas. Deployment utilizes Docker containers on Azure Container Apps with Azure PostgreSQL Flexible Server and Azure Key Vault. CI/CD is managed via GitHub Actions.
...
- **AI Transparency & Education**: Comprehensive public page at `/ai-transparency` with 14 sections covering: benefits of AI call centres, honest limitations, anti-hallucination architecture (RAG grounding, output guard, strict knowledge mode, prompt injection detection), RAG explained simply, 7 safety layers, human-in-the-loop design, multi-agent orchestration, voice AI vs text AI, cost transparency, data privacy & sovereignty, knowledge base preparation guide (non-technical), LLM best practices, performance metrics guide, and myth-busting. Serves as both educational content and compliance declaration. Sticky anchor navigation, responsive card layout. Linked from navbar and footer.
