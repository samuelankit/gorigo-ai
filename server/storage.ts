...
export class DatabaseStorage implements IStorage {
  ...
  async upsertRateCard(data: InsertCountryRateCard): Promise<CountryRateCard> {
    ...
    const numberType = data.numberType ?? "mobile";
    const existing = await this.getRateCard(data.countryId, data.deploymentModel, direction, numberType);
    if (existing) {
      const [updated] = await db
      ...
      return db
        ...
        .where(
          and(
            ...
            sql`${countryHolidays.date} LIKE ${year + '-%'}`
          )
        )
